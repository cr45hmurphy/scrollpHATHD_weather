...
def get_weather_data():
    ...
    if TEMP_SCALE == "F":  # Fahrenheit
        temperature = str(1.8*(parsed_cond['properties']['temperature']['value'])+32)  # string used for display purposes
        current_temp = (1.8*(parsed_cond['properties']['temperature']['value'])+32)      # string used for calculations
        wind_speed = float(parsed_cond['properties']['windSpeed']['value'])
        wind_gusts = float(parsed_cond['properties']['windGust']['value'])
        #feels_like = float(parsed_cond['properties']['temperature']['value'])
    '''else:                  # Celsius
        # temperature = str(parsed_cond['properties']['temp_c'])
        current_temp = parsed_cond['properties']['temp_c']
        wind_speed = float(parsed_cond['properties']['wind_kph'])
        wind_gusts = float(parsed_cond['properties']['wind_gust_kph'])
        feels_like = float(parsed_cond['properties']['feelslike_c'])
    '''
    ...
    humidity = parsed_cond['properties']['relativeHumidity']['value']
    precip = parsed_cond['properties']['precipitationLastHour']['value']
